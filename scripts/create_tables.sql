...
CREATE TYPE "driver_type_enum" AS ENUM ('employee', 'autonomous');

CREATE TYPE "driver_status_enum" AS ENUM ('active', 'suspended', 'dismissed');
...
CREATE TABLE if not exists  "driver" (
  ...
  driver_type driver_type_enum NOT NULL DEFAULT 'employee',
...
CREATE TYPE "vehicle_status_enum" AS ENUM ('active', 'maintenance', 'inactive', 'sold');
...
CREATE TYPE "vehicle_maintenance_status_enum" AS ENUM ('scheduled', 'completed', 'cancelled');
...
CREATE TABLE if not exists  "vehicle_maintenance" (
  ...
  status vehicle_maintenance_status_enum NOT NULL DEFAULT 'completed',
  created_at TIMESTAMP NOT NULL default now(),
  updated_at TIMESTAMP NULL,
  deleted_at TIMESTAMP NULL,
  FOREIGN KEY ("id_user") REFERENCES "user" ("id_user"),
...
CREATE TYPE "travel_billing_status_enum" AS ENUM ('pending', 'paid', 'overdue', 'cancelled');
...
CREATE TABLE if not exists  "travel_billing" (
  ...
  status travel_billing_status_enum NOT NULL DEFAULT 'pending',
...
CREATE TYPE "driver_payment_payment_type_enum" AS ENUM (
  'salary',
  'commission',
  'travel',
  'mileage',
...
CREATE TABLE if not exists  "driver_payment" (
  ...
  value NUMERIC(12, 2) NOT null check (value > 0),
  date DATE NOT NULL,
  payment_type driver_payment_payment_type_enum NOT NULL,
  created_at TIMESTAMP NOT NULL default now(),
  updated_at timestamp null,
  deleted_at TIMESTAMP NULL,
  FOREIGN KEY ("id_user") REFERENCES "user" ("id_user"),
